...
- `Windows/` — standalone dialogs (`ConnectionPickerWindow`, `CommandPaletteWindow`, `ShortcutsHelpWindow`, `QueryParametersWindow`, `DeleteColumnsSelectionWindow`), each with a small constructor-in/result-property-out contract.
...
`tests/DatabaseManager.Tests` covers pure/deterministic logic in both projects — 168 tests as of this writing, across:

- **Core**: the pure-logic classes listed in the table above, plus `SqlQueryAssistantService`, `TemplateStoreService`, `ExportService`, `ConnectionProfileStoreService`.
