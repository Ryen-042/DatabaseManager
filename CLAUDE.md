...
`MainWindow.xaml` + `MainWindow.xaml.cs` (~3100 lines) is still the orchestrator for most of the app — connection handling, schema browsing, SQL execution, Edit Rows state, stored procedure runner, and most event handlers live there as one code-behind class. MVVM migration is incremental and in progress, tab by tab; the Templates tab is the first one fully extracted (see `ViewModels/`/`Views/` below) — don't assume a ViewModel/View exists for any other tab just because the folders do.
...
- `ViewModels/` — `MainWindowViewModel` (owns `IsDarkMode`/`IsSchemaAssistantVisible` + exposes `CommandRegistry` and `TemplatesPanel`; MainWindow still does the actual visual work for everything not yet extracted via constructor-supplied callbacks), `ToastService`/`IToastService`/`ToastNotificationViewModel` (bottom-right auto-dismissing toasts, wired into connection and query/procedure execution failures alongside the existing status bar text), `TemplatesPanelViewModel`/`TemplateItemViewModel` (Templates tab: Save/Refresh/Delete commands against `ITemplateStoreService`, plus an `ActivateSelected()` method for double-click-to-load — construction takes callbacks from `MainWindow` for reading the current SQL text, applying an activated template, confirming deletes, and posting status text, so it has no direct dependency on `MainWindow` or any XAML element).
- `Views/` — `TemplatesPanelView` (XAML `UserControl` for the Templates tab, `DataContext="{Binding TemplatesPanel}"` set in `MainWindow.xaml`; its code-behind is only a `MouseDoubleClick` passthrough to `TemplatesPanelViewModel.ActivateSelected()`, since `ListBox` has no built-in double-click command). The pattern for future tab extractions: a `UserControl` in `Views/` bound to a `[Tab]ViewModel` property on `MainWindowViewModel`, wired into `MainWindow.xaml` as `<views:XView DataContext="{Binding [Tab]} />` inside the existing `TabItem`.
