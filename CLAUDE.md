...
`MainWindow.xaml` + `MainWindow.xaml.cs` (~3100 lines) is still the orchestrator for almost everything — connection handling, schema browsing, SQL execution, Edit Rows state, stored procedure runner, and most event handlers live there as one code-behind class. MVVM migration is incremental and in progress; `Views/` is still empty scaffolding. Don't assume a ViewModel exists for a feature just because the folder does.
...
- `ViewModels/` — `MainWindowViewModel` (owns `IsDarkMode`/`IsSchemaAssistantVisible` + exposes `CommandRegistry`; MainWindow still does the actual visual work via constructor-supplied callbacks, since the rest of the window isn't on MVVM yet), `ToastService`/`IToastService`/`ToastNotificationViewModel` (bottom-right auto-dismissing toasts, wired into connection and query/procedure execution failures alongside the existing status bar text).
